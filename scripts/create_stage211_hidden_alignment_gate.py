...
def _summary(
    ...
    result: dict[str, Any] = {
        ...
        "relative_loss_reduction": (baseline_loss - candidate_loss) / max(
            baseline_loss,
            1.0e-12,
        ),
        ...
        "cosine_improved_layers": sum(
            float(candidate[str(index)]["cosine"])
            > float(baseline[str(index)]["cosine"])
            for index in LAYER_IDS
        ),
        "weak_bands": {},
    }
    for name, layer_ids in WEAK_BANDS.items():
        ...
        result["weak_bands"][name] = {
            ...
            "relative_loss_reduction": (
                (weak_baseline_loss - weak_candidate_loss)
                / max(weak_baseline_loss, 1.0e-12)
            ),
...
def build_gate(
    *,
    ...
    if phase == "block":
        ...
        decoder_gate_passed = (
            decoder_hidden["candidate_loss"] < decoder_hidden["baseline_loss"]
        )
