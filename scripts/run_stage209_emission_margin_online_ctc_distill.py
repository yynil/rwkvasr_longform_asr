...
RUN_NAME = (
    ...
    "seqctc_ramp_20k_lr5e8_4x4090"
)
DEFAULT_OUTPUT_DIR = Path("/tmp/rwkvasr_runs") / RUN_NAME
DEFAULT_CONFIG_DIR = REPO_ROOT / "configs" / "generated" / "stage209_emission_margin_distill"
...
# Keep the already validated frame/top-k objectives fixed. Only the sequence
# objective and the teacher-token-versus-blank margin are ramped.
RAMP_PHASES: tuple[RampPhase, ...] = (
    RampPhase("ramp1", 5_000, 0.50, 0.25),
    RampPhase("ramp2", 10_000, 0.625, 0.35),
    RampPhase("ramp3", 15_000, 0.75, 0.50),
    RampPhase("ramp4", 20_000, 1.00, 0.50),
)


def _latest_exported_checkpoint(stage208_output_dir: Path) -> Path:
    latest_path = stage208_output_dir / "latest_checkpoint.yaml"
    if not latest_path.is_file():
        raise FileNotFoundError(f"Stage208 latest checkpoint metadata not found: {latest_path}")
    state = load_yaml(latest_path)
    checkpoint_path = state.get("checkpoint_path") if isinstance(state, dict) else None
    if not checkpoint_path:
        raise ValueError(f"Stage208 latest checkpoint metadata has no checkpoint_path: {latest_path}")
    checkpoint = Path(str(checkpoint_path))
    if not checkpoint.is_file():
        raise FileNotFoundError(f"Stage208 exported checkpoint not found: {checkpoint}")
    return checkpoint
    ...
    config.update(
        {
            ...
            "resume_tag": None,
            "save_every": 1 if smoke else 5_000,
            "step_eval_every": 1 if smoke else 5_000,
            "step_eval_samples": 4 if smoke else 512,
            ...
            "ctc_teacher_online_nonblank_margin": 1.0,
            ...
            "ctc_teacher_online_nonblank_window_topk_loss_weight": 0.50,
...
def _write_config(config_dir: Path, phase: RampPhase, config: dict[str, Any], *, smoke: bool) -> Path:
    ...
    path = target_dir / f"stage209_{phase.name}.yaml"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Stage209 emission-margin online Nano-CTC distillation."
    )
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--config-dir", type=Path, default=DEFAULT_CONFIG_DIR)
    parser.add_argument("--stage208-output-dir", type=Path, default=STAGE208_OUTPUT_DIR)
    ...
    init_checkpoint = args.init_checkpoint or _latest_exported_checkpoint(args.stage208_output_dir)
