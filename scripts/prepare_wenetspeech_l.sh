...
INPUT_ROOT="${1:-/media/usbhd/training_data/asr/wenet-e2e/wenetspeech/data}"
OUTPUT_ROOT="${2:-/media/usbhd/training_data/asr/wenet-e2e/wenetspeech/webdataset_l_train}"
...
INPUT_GLOB="${WENETSPEECH_INPUT_GLOB:-cuts_L_fixed.*.jsonl.gz}"
...
echo "[rwkvasr] Converting WenetSpeech Lhotse cuts to WebDataset"
echo "[rwkvasr] input=${INPUT_ROOT} glob=${INPUT_GLOB} output=${OUTPUT_ROOT}"
cargo run --release --manifest-path tools/Cargo.toml --bin convert_asr_corpus -- wenetspeech-lhotse \
...
echo "[rwkvasr] WenetSpeech preprocessing complete."
