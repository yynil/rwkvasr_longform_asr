...
stage211_json_matches() {
  ...
  guarded_filter="[(${filter})] as \$stage211_results | (\$stage211_results | length) == 1 and \$stage211_results[0] == true"
  [[ -s "${path}" ]] && jq -e "${guarded_filter}" "${path}" >/dev/null 2>&1
}
...
stage211_alignment_promotion_results_valid() {
  local path="$1"
  stage211_json_matches "${path}" '
    def nondivergent_mixer:
      .stage == "mixer" and .phase_gate_passed == true and
      (.strict_metric_gate_passed | type) == "boolean" and
      .alignment_loss_nondivergence as $loss |
      $loss.schema_version == 1 and $loss.pipeline == "stage211" and
      $loss.artifact == "alignment_loss_nondivergence" and $loss.gate_passed == true and
      ([$loss.baseline_loss, $loss.candidate_loss, $loss.candidate_to_baseline_ratio] |
        all(.[]; type == "number" and isfinite)) and
      $loss.baseline_loss > 0 and $loss.candidate_loss >= 0 and $loss.maximum_ratio == 1.2 and
      $loss.baseline_loss == .fixed.baseline_eval_loss and
      $loss.candidate_loss == .fixed.candidate_eval_loss and
      $loss.candidate_to_baseline_ratio == ($loss.candidate_loss / $loss.baseline_loss) and
      $loss.candidate_to_baseline_ratio <= $loss.maximum_ratio;
    .coverage_results as $coverage |
    [.alignment_results[] | . as $alignment |
      [$coverage[] | select(.stage == $alignment.stage)] as $matches |
      ($matches | length) == 1 and $matches[0] as $record |
      ($record.correction_rounds | type) == "number" and
      $record.correction_round_promotion.gate_passed == true and
      $record.correction_round_promotion.completed_rounds == $record.correction_rounds and
      $record.correction_round_promotion.guaranteed_rounds == 3 and
      (if (.promotion_policy // "strict_quality") == "strict_quality" then
        .gate_passed == true and .stratified_gate_passed == true and
        (.strict_metric_gate_passed != false) and $record.correction_rounds >= 3
      elif .promotion_policy == "complete_coverage_nondivergent_loss" then
        nondivergent_mixer and $record.correction_rounds == 0 and
        $record.correction_round_promotion.correction_started == false
      else false end)
    ] | length == 3 and all'
}
...
stage211_trajectory_results_valid() {
  local path="$1"
  stage211_alignment_promotion_results_valid "${path}" || return 1
  stage211_json_matches \
    "${path}" \
    '(.alignment_results | length) == 3 and ([.alignment_results[] | .stage] == ["mixer", "block", "logits"]) and ([.alignment_results[] | .promotion_policy as $policy | .trajectory_retention | ((.gate_passed | type) == "boolean" and (.gate_passed == true or ($policy == "complete_coverage_nondivergent_loss" and .required_for_promotion == false)) and .fixed_eval_samples == 256 and .terminal_entries >= 5 and .source_order[:5] == ["easy", "medium", "hard", "long", "supplemental_natural"] and .max_relative_regression_pct == 10.0 and ([.best_prior_loss, .candidate_loss, .relative_regression_pct] | all(.[]; type == "number" and isfinite)))] | all)'
}
...
stage211_full_labeled_profile_valid() {
  ...
  stage211_json_matches \
    "${path}" \
    '.ctc_label_proof as $labels | ([.coverage_results[] | select(.stage == "sft")] | length) == 1 and (.coverage_results[] | select(.stage == "sft")) as $sft | $labels.labeled_profile_schema_version == 2 and ($labels.labeled_profile_receipt_path | type) == "string" and ($labels.labeled_profile_receipt_sha256 | length) == 64 and $labels.all_accepted_unique_rows_required == true and $labels.source_language_interleave_required == true and $labels.train_samples > 0 and $labels.eval_samples > 0 and $labels.total_samples == ($labels.train_samples + $labels.eval_samples) and $labels.unique_utterance_ids == $labels.total_samples and $labels.pronunciation_target_samples == $labels.total_samples and $labels.ctc_feasible_samples == $labels.total_samples and $labels.ctc_tokens > 0 and $labels.ctc_unk_tokens == 0 and $labels.ctc_forbidden_tokens == 0 and ([$labels.source_counts[]] | add) == $labels.total_samples and ([$labels.language_counts[]] | add) == $labels.total_samples and $labels.public_clean_rebuild_passed == true and ($labels.public_clean_rebuild_receipt_sha256 | length) == 64 and ($labels.public_clean_source_profile_sha256 | length) == 64 and $labels.public_clean_exclusion_reason == "public_evaluation_overlap_exact_encoded_audio" and $labels.public_clean_exclusions_rows == 21 and $labels.public_clean_rejected_training_overlap_rows == 21 and $labels.public_clean_rejected_internal_eval_overlap_rows == 0 and $labels.public_audio_isolation_passed == true and ($labels.public_audio_overlap_receipt_sha256 | length) == 64 and $labels.public_audio_comparison_mode == "exact_encoded_audio_bytes" and $labels.public_audio_size_prefilter_lossless_for_exact_bytes == true and $labels.public_audio_scanned_rows == $labels.total_samples and $labels.public_audio_public_rows == 52436 and $labels.public_audio_training_overlap_rows == 0 and $labels.public_audio_internal_eval_overlap_rows == 0 and $labels.public_audio_normalized_pcm_complete == false and $labels.public_audio_near_duplicate_complete == false and $sft.unique_or_train_rows == $labels.train_samples and $sft.evaluation_rows == $labels.eval_samples and $sft.hours_per_epoch > 0 and $sft.epochs == 1 and $sft.row_exposures == $labels.train_samples and $sft.hour_exposures == $sft.hours_per_epoch and $sft.steps > 0 and $sft.tail_padding_sample_exposures >= 0 and $sft.executed_sample_exposures == ($sft.row_exposures + $sft.tail_padding_sample_exposures) and $sft.step_eval_cadence.interval_steps == 2000 and $sft.step_eval_cadence.source_order == ["labeled_sft"] and $sft.step_eval_cadence.source_count == 1 and $sft.step_eval_cadence.total_reports == (($sft.steps + 1999) / 2000 | floor) and $sft.step_eval_cadence.sources[0].terminal_step == $sft.steps and $sft.step_eval_cadence.sources[0].report_count == $sft.step_eval_cadence.total_reports'
}
...
stage211_sft_correction_coverage_valid() {
  local path="$1"
  stage211_json_matches \
    "${path}" \
    '([.coverage_results[] | select(.stage == "sft")] | length) == 1 and (.coverage_results[] | select(.stage == "sft")) as $sft | .sft_correction_evidence as $correction | $sft.correction == $correction and $correction.schema_version == 1 and ($correction.applied | type) == "boolean" and $sft.correction_rounds == $correction.rounds and $sft.correction_row_exposures == $correction.row_exposures and $sft.correction_hour_exposures == $correction.hour_exposures and $sft.correction_steps == $correction.steps and $sft.correction_tail_padding_sample_exposures == $correction.tail_padding_sample_exposures and $sft.correction_executed_sample_exposures == $correction.executed_sample_exposures and $sft.effective_row_exposures == ($sft.row_exposures + $correction.row_exposures) and $sft.effective_hour_exposures == ($sft.hour_exposures + $correction.hour_exposures) and $sft.effective_steps == ($sft.steps + $correction.steps) and $sft.effective_tail_padding_sample_exposures == ($sft.tail_padding_sample_exposures + $correction.tail_padding_sample_exposures) and $sft.effective_executed_sample_exposures == ($sft.executed_sample_exposures + $correction.executed_sample_exposures) and (if $correction.applied == false then $correction.rounds == 0 and $correction.unique_rows_per_round == 0 and $correction.row_exposures == 0 and $correction.hour_exposures == 0 and $correction.steps == 0 and $correction.tail_padding_sample_exposures == 0 and $correction.executed_sample_exposures == 0 and $correction.language_row_exposures == {} and $correction.source_row_exposures == {} and $correction.round_receipts == [] else ($correction.rounds >= 1 and $correction.rounds <= 3) and $correction.unique_rows_per_round > 0 and $correction.row_exposures == ($correction.rounds * $correction.unique_rows_per_round) and $correction.hour_exposures > 0 and $correction.steps > 0 and $correction.tail_padding_sample_exposures >= 0 and $correction.executed_sample_exposures == ($correction.row_exposures + $correction.tail_padding_sample_exposures) and ([range(1; $correction.rounds + 1)] == [$correction.round_receipts[].round]) and ($correction.round_receipts | length) == $correction.rounds and ([ $correction.round_receipts[] | .row_exposures ] | add) == $correction.row_exposures and ([ $correction.round_receipts[] | .hour_exposures ] | add) == $correction.hour_exposures and ([ $correction.round_receipts[] | .steps ] | add) == $correction.steps and ([ $correction.round_receipts[] | .tail_padding_sample_exposures ] | add) == $correction.tail_padding_sample_exposures and ([ $correction.round_receipts[] | .executed_sample_exposures ] | add) == $correction.executed_sample_exposures and ([ $correction.language_row_exposures[] ] | add) == $correction.row_exposures and ([ $correction.source_row_exposures[] ] | add) == $correction.row_exposures and ($correction.full_sft_completion_sha256 | length) == 64 and ($correction.correction_profile_sha256 | length) == 64 and ($correction.initial_checkpoint_sha256 | length) == 64 and ($correction.final_checkpoint_sha256 | length) == 64 and ([ $correction.round_receipts[] | ((.receipt_sha256 | length) == 64 and (.init_checkpoint_sha256 | length) == 64 and (.completion_checkpoint_sha256 | length) == 64) ] | all) end)'
}
...
stage211_final_candidate_valid() {
  ...
  if ! stage211_sft_correction_coverage_valid "${final_stepwise_report}"; then
    stage211_log "final Stage211 SFT correction-coverage proof failed validation source=${final_report}"
    return 1
  fi
}
