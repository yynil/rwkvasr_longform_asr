...
try:
    from scripts.run_stage210m_sequence_ctc_repair import (
        DEFAULT_BUCKET_MANIFEST,
        DEFAULT_OUTPUT_ROOT,
        _config as _stage210m_config,
        _latest_step,
        _run,
        _segments as _stage210m_segments,
    )
except ModuleNotFoundError as error:
    if error.name != "scripts":
        raise
    from run_stage210m_sequence_ctc_repair import (
        DEFAULT_BUCKET_MANIFEST,
        DEFAULT_OUTPUT_ROOT,
        _config as _stage210m_config,
        _latest_step,
...
DEFAULT_CONFIG_DIR = REPO_ROOT / "configs" / "generated" / "stage210n_full_easy_spike_tolerant"
FULL_EASY_STEPS = 30_064
EVAL_INTERVAL = 10_000


def _default_output_dir() -> Path:
    return DEFAULT_OUTPUT_ROOT / (
        "sensevoice_rwkv_stage210n_stage210m1000_sourcebalanced_easy1490h_1ep_"
        "sequence0p2_condwindow0p25_stackedhard12_lr3e7_wd0_4x4090"
    )


def _segments(*, smoke: bool) -> list[dict[str, Any]]:
    source = _stage210m_segments(smoke=bool(smoke))[0]
    target_step = 2 if smoke else FULL_EASY_STEPS
    return [
        {
            **source,
            "name": "spike_tolerant_smoke_2steps" if smoke else "full_easy_30064steps",
    ...
    config.update(
        {
            ...
            "lr": 3.0e-7,
            "weight_decay": 0.0,
            "save_every": target_step if smoke else EVAL_INTERVAL,
            "step_eval_every": target_step if smoke else EVAL_INTERVAL,
            "top_k_step_checkpoints": 1 if smoke else 4,
            "ctc_teacher_online_conditional_nonblank_hard_loss_weight": 0.125,
            "ctc_teacher_online_sequence_loss_weight": 0.20,
...
def _write_config(
    *,
    ...
    target_dir = config_dir / ("smoke" if smoke else "full_easy")
    target_dir.mkdir(parents=True, exist_ok=True)
    path = target_dir / f"stage210n_{segment['name']}.yaml"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run full-easy Nano CTC alignment with spike-tolerant conditional token loss."
    )
    ...
    if latest_step <= 0:
        if init_checkpoint is None:
            parser.error("a fresh Stage210n run requires an explicitly selected checkpoint")
        if not init_checkpoint.is_file():
            raise FileNotFoundError(str(init_checkpoint))
    if init_checkpoint is None:
        init_checkpoint = output_dir / "resume-placeholder.pt"

    print("phase=full_easy_spike_tolerant_output_alignment", flush=True)
    ...
    for segment in _segments(smoke=bool(args.smoke)):
        ...
        if not args.dry_run and latest_step < target_step:
            raise RuntimeError(
                f"Stage210n exited before target_step={target_step}: latest_step={latest_step}"
            )
    if not args.dry_run:
