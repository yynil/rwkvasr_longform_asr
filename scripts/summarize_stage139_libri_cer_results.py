...
def _load_bucket_map(lengths_dir: Path) -> dict[str, str]:
    ...
    for path in sorted(lengths_dir.glob("libri_*.lengths.jsonl")):
        for row in _iter_jsonl(path):
            bucket = _as_key(row.get("_stage139_bucket"))
            if not bucket:
                if path.name == "libri_short.lengths.jsonl":
                    bucket = "short"
                elif path.name == "libri_medium.lengths.jsonl":
                    bucket = "medium"
                elif path.name == "libri_long.lengths.jsonl":
                    bucket = "long"
            if not bucket or bucket == "cer_guard":
                continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize Stage139 LibriSpeech CER guard predictions.")
    ...
    combined_path = prediction_dir / "libri_cer_guard.ctc.jsonl"
    ...
    lines = [
        "# Stage139 Libri CER Guard Metrics",
        "",
        "| split | samples | WER | CER | ref words | ref chars |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            ...
            f"{row.get('ref_words', 'n/a')} | {row.get('ref_chars', 'n/a')} |"
        )
