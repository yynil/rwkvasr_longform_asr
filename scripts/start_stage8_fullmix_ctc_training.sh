...
BASE_CONFIG="${BASE_CONFIG:-${REPO_ROOT}/configs/generated/sensevoice_rwkv_stage7_full_ctc_draft_ar_from_stage7_werbest_4x4090_deepspeed.yaml}"
INIT_CHECKPOINT_PATH="${INIT_CHECKPOINT_PATH:-/media/usbhd/rwkvasr_runs/sensevoice_rwkv_stage7_full_ctc_draft_ar_from_stage7_werbest_ctc0p4_ar0p6_bs4_lr2e5_zero1_nockpt_4x4090/epoch-1.pt}"

FULLMIX_ROOT="${FULLMIX_ROOT:-/media/usbhd/training_data/asr/mix/gigaspeech_xl_wenetspeech_l_webdataset}"
FULLMIX_LENGTH_INDEX="${FULLMIX_LENGTH_INDEX:-${FULLMIX_ROOT}/webdataset_lengths.jsonl}"
FULLMIX_BUCKET_MANIFEST="${FULLMIX_BUCKET_MANIFEST:-${FULLMIX_ROOT}/webdataset_buckets_audio_text/manifest.json}"

RUN_DIR="${RUN_DIR:-/media/usbhd/rwkvasr_runs/sensevoice_rwkv_stage8_fullmix_ctcnum_from_stage7_epoch1_bs12_lr2e5_zero1_nockpt_4x4090}"
CONFIG_PATH="${CONFIG_PATH:-${REPO_ROOT}/configs/generated/sensevoice_rwkv_stage8_fullmix_ctcnum_from_stage7_epoch1_4x4090_deepspeed.yaml}"
...
SIDECAR_SESSION="${SIDECAR_SESSION:-sidecar_stage8_fullmix_ctc}"
...
log() {
  printf '[stage8-fullmix-ctc] %(%Y-%m-%d %H:%M:%S)T %s\n' -1 "$*"
}
...
write_config() {
  ...
  if [[ ! -s "${FULLMIX_LENGTH_INDEX}" ]]; then
    echo "full mix length index missing: ${FULLMIX_LENGTH_INDEX}" >&2
    exit 1
  fi
  if [[ ! -s "${FULLMIX_BUCKET_MANIFEST}" ]]; then
    echo "full mix bucket manifest missing: ${FULLMIX_BUCKET_MANIFEST}" >&2
    exit 1
  fi

  export BASE_CONFIG INIT_CHECKPOINT_PATH FULLMIX_ROOT FULLMIX_LENGTH_INDEX FULLMIX_BUCKET_MANIFEST
...
cfg["webdataset_root"] = os.environ["FULLMIX_ROOT"]
cfg["webdataset_index_path"] = str(Path(os.environ["FULLMIX_ROOT"]) / "webdataset_index.json")
cfg["webdataset_length_index_path"] = os.environ["FULLMIX_LENGTH_INDEX"]
cfg["webdataset_bucket_manifest_path"] = os.environ["FULLMIX_BUCKET_MANIFEST"]
...
print(f"saved_config={os.environ['CONFIG_PATH']} run_dir={run_dir}")
PY
  "${PYTHON_BIN}" - <<'PY'
import math
import os
...
cfg = dict(load_yaml(os.environ["CONFIG_PATH"]))
expected = {
...
if errors:
    print("Stage8/12 direction dropout guard failed: " + "; ".join(errors), file=sys.stderr)
    sys.exit(1)
print(
    "Stage8/12 direction dropout guard passed: "
