...
DEFAULT_CONFIG_DIR = REPO_ROOT / "configs" / "generated" / "stage210j_decoupled_hidden_ctc_distill"


def _default_output_dir() -> Path:
    return DEFAULT_OUTPUT_ROOT / (
        "sensevoice_rwkv_stage210j_stage210h2000_sourcebalanced_easy_"
        "decoupled_hidden24_boundaries_factorized_admission2k_lr2e7_wd0_4x4090"
    )
    ...
    config.update(
        {
            ...
            "ctc_teacher_online_layer_sample_count": 24,
...
def _write_config(
    *,
    ...
    path = target_dir / f"stage210j_{segment['name']}.yaml"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run source-balanced Nano CTC distillation with balanced hidden frames, "
            "all-frame blank calibration, and increased layer coverage."
        )
    )
    ...
    if latest_step <= 0:
        if init_checkpoint is None:
            parser.error("a fresh Stage210j run requires --init-checkpoint from Stage210h")
    ...
    for segment in _segments(smoke=bool(args.smoke)):
        ...
        if latest_step < target_step:
            raise RuntimeError(
                f"Stage210j exited early: latest_step={latest_step} target_step={target_step}"
            )
