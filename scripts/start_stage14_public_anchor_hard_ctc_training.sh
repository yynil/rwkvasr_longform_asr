...
BASE_CONFIG="${BASE_CONFIG:-${REPO_ROOT}/configs/generated/sensevoice_rwkv_stage13_fullmix_ctc_from_stage12b_werbest_4x4090_deepspeed.yaml}"
...
CURRICULUM_ROOT="${CURRICULUM_ROOT:-/media/usbhd/training_data/asr/curriculum/stage14_public_anchor_hard_mix}"
STAGE_NAME="${STAGE_NAME:-stage14_public50_hard50}"
TARGET_SAMPLES="${TARGET_SAMPLES:-800000}"
CLEAN_RATIO="${CLEAN_RATIO:-0.50}"
MIX_EVAL_RATIO="${MIX_EVAL_RATIO:-0.001}"
MIX_SEED="${MIX_SEED:-20260616}"
BUCKET_WIDTH="${BUCKET_WIDTH:-80}"

RUN_DIR="${RUN_DIR:-/media/usbhd/rwkvasr_runs/sensevoice_rwkv_stage14_public50_hard50_ctc_from_stage12b_werbest_bs12_lr2e6_zero1_nockpt_noaug_nodirdrop_4x4090}"
CONFIG_PATH="${CONFIG_PATH:-${REPO_ROOT}/configs/generated/sensevoice_rwkv_stage14_public50_hard50_ctc_from_stage12b_werbest_4x4090_deepspeed.yaml}"
...
SIDECAR_SESSION="${SIDECAR_SESSION:-sidecar_stage14_public_anchor_hard_ctc}"
SIDECAR_LIMIT="${SIDECAR_LIMIT:-96}"
SIDECAR_SOURCE_QUOTAS="${SIDECAR_SOURCE_QUOTAS:-clean_librispeech:librispeech_*.tar:12,clean_aishell:aishell3_*.tar:12,clean_cv_en:commonvoice_en_*.tar:12,clean_cv_cn:commonvoice_cn_*.tar:12,gigaspeech:GSXL-*.tar:24,wenetspeech:WSL-*.tar:24}"
...
log() {
  printf '[stage14-public-anchor-hard-ctc] %(%Y-%m-%d %H:%M:%S)T %s\n' -1 "$*"
}
...
if errors:
    raise SystemExit("Stage14 direction dropout guard failed: " + "; ".join(errors))
print(f"saved_config={os.environ['CONFIG_PATH']} run_dir={run_dir}")
print("Stage14 direction dropout guard passed: direction_variant=none p_start=0.0 p_max=0.0 warmup_steps=0 ramp_steps=0")
PY
}
