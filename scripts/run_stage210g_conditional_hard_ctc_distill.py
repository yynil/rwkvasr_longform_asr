...
try:
    from scripts.run_stage210f_factorized_ctc_distill import (
        DEFAULT_OUTPUT_ROOT,
        _config as _stage210f_config,
        _latest_step,
        _run,
        _segments,
    )
    ...
    if error.name != "scripts":
        raise
    from run_stage210f_factorized_ctc_distill import (
        DEFAULT_OUTPUT_ROOT,
        _config as _stage210f_config,
        _latest_step,
        _run,
...
DEFAULT_CONFIG_DIR = REPO_ROOT / "configs" / "generated" / "stage210g_conditional_hard_ctc_distill"
...
def _write_config(
    *,
    ...
    path = target_dir / f"stage210g_{segment['name']}.yaml"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Stage210g factorized soft/hard online Nano CTC distillation."
    )
    ...
    if latest_step <= 0:
        if init_checkpoint is None:
            parser.error("a fresh Stage210g run requires --init-checkpoint from Stage210f")
    ...
    for segment in _segments(str(args.phase), smoke=bool(args.smoke)):
        ...
        if latest_step < target_step:
            raise RuntimeError(
                f"Stage210g exited early: latest_step={latest_step} target_step={target_step}"
            )
        if args.only_next:
            break
