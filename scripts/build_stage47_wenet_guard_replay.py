...
DEFAULT_OUTPUT_DIR = "/media/usbhd/rwkvasr_runs/stage47_wenet_guard_repair_20260620"
CANONICAL_SOURCES = (
    "librispeech",
    "aishell3",
    "commonvoice_en",
...
def _difficulty_tier(error: float, q25: float, q75: float) -> str:
    if error <= q25:
        return "easy_anchor"
    if error <= q75:
        return "medium"
...
def _build_original_replay(
    *,
    ...
    for row in _iter_jsonl(stage22_lengths):
        ...
        enriched = dict(row)
        enriched["_stage47_component"] = "original_replay"
        enriched["_stage47_source"] = source
        enriched["_stage47_selector"] = "stage22_source_balanced_original_label"
    ...
    for source in CANONICAL_SOURCES:
        ...
        if len(bucket) < target:
            ...
            for row in fill:
                row["_stage47_original_replacement_fill"] = True
    ...
    summary = {
        ...
        "actual_by_source": dict(sorted(Counter(str(row["_stage47_source"]) for row in rows).items())),
    }
    return rows, summary
...
def _build_teacher_component(
    *,
    ...
    for cache_row in _iter_jsonl(teacher_cache_path):
        ...
        if source == "wenetspeech":
            skipped_wenet += 1
            continue
        if source not in CANONICAL_SOURCES or source == "unknown":
            skipped_unknown += 1
            continue
        ...
        enriched["_stage47_component"] = "teacher_repair_non_wenet"
        enriched["_stage47_source"] = source
        enriched["_stage47_teacher_override"] = True
        enriched["_stage47_primary_improvement"] = float(cache_row.get("primary_improvement", 0.0) or 0.0)
        enriched["_stage47_student_error"] = float(
            cache_row.get("student_wer", cache_row.get("student_cer", 0.0)) or 0.0
        )
        enriched["_stage47_teacher_error"] = float(
            cache_row.get("teacher_wer", cache_row.get("teacher_cer", 0.0)) or 0.0
        )
        available_by_source[source].append(enriched)

    non_wenet_sources = [source for source in CANONICAL_SOURCES if source != "wenetspeech"]
    targets = _split_even(target_total, non_wenet_sources)
    sampled: list[dict[str, Any]] = []
    for source in non_wenet_sources:
    ...
    if any(_canonical_source(row.get("source")) == "wenetspeech" for row in sampled_cache_rows):
        raise ValueError("Stage47 teacher override cache unexpectedly contains WenetSpeech rows")
    ...
    summary = {
        ...
        "actual_by_source": dict(sorted(Counter(str(row["_stage47_source"]) for row in sampled).items())),
        ...
        "skipped_wenet_cache_rows": skipped_wenet,
        "skipped_unknown_cache_rows": skipped_unknown,
...
def _build_wenet_anchor_component(
    *,
    ...
    for score_row in _iter_jsonl(student_scored):
        ...
        enriched = dict(length_row)
        enriched["_stage47_component"] = "wenet_original_anchor"
        enriched["_stage47_source"] = "wenetspeech"
        enriched["_stage47_teacher_override"] = False
        enriched["_stage47_primary_error"] = error
        enriched["_stage47_student_wer"] = float(score_row.get("student_wer", error) or error)
        enriched["_stage47_student_cer"] = float(score_row.get("student_cer", error) or error)
        enriched["_stage47_pred_text"] = score_row.get("pred_text")
        candidates.append(enriched)

    if not candidates:
        raise ValueError("no WenetSpeech candidate rows are available for Stage47 anchors")
    errors = sorted(float(row["_stage47_primary_error"]) for row in candidates)
    q25 = _quantile(errors, 0.25)
    max_q = _quantile(errors, max_quantile_fraction)
    anchor_pool: list[dict[str, Any]] = []
    for row in candidates:
        row = dict(row)
        row["_stage47_wenet_anchor_tier"] = _difficulty_tier(
            float(row["_stage47_primary_error"]),
            q25,
            max_q,
        )
        if float(row["_stage47_primary_error"]) <= max_q:
            anchor_pool.append(row)
    if not anchor_pool:
        raise ValueError("WenetSpeech anchor pool is empty after quantile filtering")

    sampled = _sample_with_replacement(rng, anchor_pool, target_total)
    summary = {
        "target_total": target_total,
        "available_candidates": len(candidates),
        "available_anchor_pool": len(anchor_pool),
        "max_quantile_fraction": max_quantile_fraction,
        "q25": q25,
        "max_quantile": max_q,
        "actual_samples": len(sampled),
        "unique_sampled_anchor_ids": len({_utt_id(row) for row in sampled}),
        "sampled_by_tier": dict(
            sorted(Counter(str(row["_stage47_wenet_anchor_tier"]) for row in sampled).items())
        ),
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build Stage47 Wenet-guard replay length index and non-Wenet teacher cache."
    )
    ...
    parser.add_argument("--output-name", default="stage47_original180k_teacher8k_wenet12k.lengths.jsonl")
    parser.add_argument("--teacher-cache-name", default="stage47_teacher_non_wenet_sampled.cache.jsonl")
    parser.add_argument("--summary-name", default="stage47_original180k_teacher8k_wenet12k.summary.json")
    parser.add_argument("--original-samples", type=int, default=180000)
    parser.add_argument("--teacher-samples", type=int, default=8000)
    parser.add_argument("--wenet-anchor-samples", type=int, default=12000)
    parser.add_argument("--wenet-max-quantile", type=float, default=0.75)
...
def main() -> None:
    ...
    original_exclude_ids = selector_ids | accepted_teacher_ids
    original_rows, original_summary = _build_original_replay(
        ...
        exclude_ids=original_exclude_ids,
        rng=rng,
    )
    wenet_exclude_ids = selector_ids | accepted_teacher_ids
    wenet_rows, wenet_summary = _build_wenet_anchor_component(
        candidate_lengths=candidate_lengths,
        student_scored=student_scored,
        target_total=int(args.wenet_anchor_samples),
        exclude_ids=wenet_exclude_ids,
        rng=rng,
        max_quantile_fraction=float(args.wenet_max_quantile),
    )

    combined_rows = original_rows + teacher_rows + wenet_rows
    ...
    component_counts = Counter(str(row["_stage47_component"]) for row in combined_rows)
    source_counts = Counter(str(row["_stage47_source"]) for row in combined_rows)
    accidental_wenet_override = [
        _utt_id(row)
        for row in teacher_cache_rows
        if _canonical_source(row.get("source")) == "wenetspeech"
    ]
    if accidental_wenet_override:
        raise ValueError(f"Wenet rows in teacher cache: {accidental_wenet_override[:5]}")
    ...
    summary = {
        ...
        "accepted_teacher_ids_excluded_from_original_and_wenet": len(accepted_teacher_ids),
        "unique_output_utt_ids": len({_utt_id(row) for row in combined_rows}),
        "original_replay": original_summary,
        "teacher_repair_non_wenet": teacher_summary,
        "wenet_original_anchor": wenet_summary,
    }
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(
        "[rwkvasr] stage47 Wenet-guard replay ready "
