...
try:
    from scripts.run_stage210k_timemixer_hidden_realign import (
        DEFAULT_BUCKET_MANIFEST,
        DEFAULT_OUTPUT_ROOT,
        _config as _stage210k_config,
        _latest_step,
        _run,
        _segments as _stage210k_segments,
    )
except ModuleNotFoundError as error:
    if error.name != "scripts":
        raise
    from run_stage210k_timemixer_hidden_realign import (
        DEFAULT_BUCKET_MANIFEST,
        DEFAULT_OUTPUT_ROOT,
        _config as _stage210k_config,
        _latest_step,
...
DEFAULT_CONFIG_DIR = REPO_ROOT / "configs" / "generated" / "stage210l_stacked_hidden_realign"
...
def _default_output_dir() -> Path:
    return DEFAULT_OUTPUT_ROOT / (
        "sensevoice_rwkv_stage210l_stage210h2000_sourcebalanced_easy_"
        "stacked_timemixer_hidden8_finalenc0p5_decoder0p5_lr3e6_wd0_4x4090"
    )


def _segments(*, smoke: bool) -> list[dict[str, Any]]:
    source = _stage210k_segments(smoke=bool(smoke))[0]
    target_step = 2 if smoke else ADMISSION_STEPS
    return [
        {
            **source,
            "name": "stacked_hidden_smoke_2steps" if smoke else "stacked_hidden_1000steps",
    ...
    config.update(
        {
            ...
            "ctc_teacher_online_layer_mixer_loss_weight": 0.5,
...
def _write_config(
    *,
    ...
    path = target_dir / f"stage210l_{segment['name']}.yaml"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Realign the stacked BiRWKV encoder and frozen Nano CTC hidden path."
    )
    ...
    if latest_step <= 0:
        if init_checkpoint is None:
            parser.error("a fresh Stage210l run requires an explicitly selected initialization checkpoint")
        if not init_checkpoint.is_file():
            raise FileNotFoundError(str(init_checkpoint))
    if init_checkpoint is None:
        init_checkpoint = output_dir / "resume-placeholder.pt"

    print("phase=stacked_hidden", flush=True)
    ...
    for segment in _segments(smoke=bool(args.smoke)):
        ...
        if not args.dry_run and latest_step < target_step:
            raise RuntimeError(
                f"Stage210l exited before target_step={target_step}: latest_step={latest_step}"
            )
    if not args.dry_run:
