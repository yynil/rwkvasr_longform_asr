...
STAGE211_SFT_STEP_EVAL_INTERVAL = 2_000
...
_STAGE211_STEP_EVAL_REPORT_PATTERN = re.compile(r"^step_eval_layers_step-([1-9][0-9]*)\.yaml$")
    ...
    *,
    ...
    for layer_id in _STAGE211_ALIGNMENT_LAYER_IDS:
        ...
        if cosine_failed:
            ...
            row["cosine_regression_max"] = max(row["cosine_regression_max"], cosine_regression)
...
def _stage211_correction_focus_trajectory(
    ...
    *,
    ...
    if trajectory.get("phase") != phase or not isinstance(trajectory.get("gate_passed"), bool):
    ...
    for role, entry in (
        ...
        if not isinstance(report, dict):
            raise ValueError(
                f"Stage211 {phase} correction-focus trajectory {role} report must be an object."
            )
        if int(report.get("step", -1)) != int(entry.get("step", -2)):
            raise ValueError(f"Stage211 {phase} correction-focus trajectory {role} step mismatch.")
        sources[role] = report
        reports[role] = {
...
def _stage211_step_eval_cadence_source(
    *,
    ...
    interval = int(interval)
    expected_steps = _stage211_expected_step_eval_steps(terminal_step, interval=interval)
    ...
    if any(train_config.get(key) != value for key, value in expected_config.items()):
        raise ValueError(
            ...
            f"{interval}-step evaluation cadence."
        )
    ...
    for raw_record, step in zip(raw_records, expected_steps, strict=True):
        ...
        if Path(
            str(provenance.get("bucket_manifest_path") or "")
        ).expanduser().resolve() != manifest_path or provenance.get(
            "bucket_manifest_sha256"
        ) != record.get("bucket_manifest_sha256"):
            raise ValueError(f"Stage211 {phase}/{source_name} step {step} eval manifest mismatch.")
...
def build_stage211_sft_step_eval_cadence(
    completion: dict[str, Any],
) -> dict[str, Any]:
    if (
        completion.get("phase") != "sft"
        or completion.get("artifact") != "labeled_sft_completion"
        or completion.get("complete") is not True
        or int(completion.get("epochs", -1)) != 1
    ):
        raise ValueError("Stage211 SFT step-eval completion source is invalid.")
    terminal_step = int(completion.get("estimated_train_steps", -1))
    source_record = {
        **completion,
        "steps": terminal_step,
        "steps_per_epoch": terminal_step,
    }
    source = _stage211_step_eval_cadence_source(
        phase="sft",
        order=0,
        source_kind="labeled_sft",
        source_name="labeled_sft",
        record=source_record,
        interval=STAGE211_SFT_STEP_EVAL_INTERVAL,
    ...
    return {
        ...
        "source_count": 1,
        "total_reports": int(source["actual_report_count"]),
        "sources": [source],
    }


def validate_stage211_sft_step_eval_cadence(
    completion: dict[str, Any],
) -> dict[str, Any]:
    replayed = build_stage211_sft_step_eval_cadence(completion)
    _validate_stage211_replayed_value(
        completion.get("step_eval_cadence"),
        replayed,
        label="sft periodic fixed-eval cadence",
    )
    return replayed
...
def _stage211_trajectory_retention_entry(
    *,
    ...
    if raw_feature_seed != 0 and not legacy_missing_feature_seed:
        raise ValueError(f"Stage211 {phase}/{source_name} trajectory fixed feature seed mismatch.")
...
def build_stage211_trajectory_retention_gate(
    *,
    ...
    if supplemental_segment.get("difficulty") != STAGE211_SUPPLEMENTAL_DIFFICULTY:
        raise ValueError("Stage211 trajectory retention lacks supplemental_natural coverage.")
    corrections = list(post_coverage_corrections or [])
    sources = [("curriculum", str(segment["difficulty"]), segment) for segment in segments]
    sources.append(("curriculum", STAGE211_SUPPLEMENTAL_DIFFICULTY, supplemental_segment))
    sources.extend(
    ...
    candidate = entries[-1]
    if Path(str(candidate["checkpoint_path"])).resolve() != checkpoint_path or candidate[
        "checkpoint_sha256"
...
def _validate_stage211_correction_batch_profile(
    correction: dict[str, Any],
    *,
    phase: str,
    manifest_path: Path,
    ...
    preflight_path = _validate_bound_file(
        correction,
        path_key="batch_profile_preflight_path",
        sha256_key="batch_profile_preflight_sha256",
        label=f"Stage211 {phase} correction batch-profile preflight",
    )
    preflight = validate_stage211_batch_profile_preflight(
        preflight_path,
        phase=phase,
        require_candidate=False,
    )
    if (
        Path(str(preflight["init_checkpoint_path"])).resolve() != init_checkpoint
        or Path(str(preflight["bucket_manifest_path"])).resolve() != manifest_path
    ):
        raise ValueError(f"Stage211 {phase} correction batch-profile inputs changed.")
    admission_path_value = correction.get("batch_profile_admission_path")
    admission_sha256_value = correction.get("batch_profile_admission_sha256")
    if admission_path_value is None and admission_sha256_value is None:
        admission = None
        selected_profile = preflight["selected_profile_row"]["profile"]
        if (
            preflight["selection_decision"] != "keep_baseline"
            or phase == "logits"
        ...
        if admission["receipt_sha256"] != admission_sha256_value:
            raise ValueError(f"Stage211 {phase} correction batch-profile admission changed.")
        selected_profile = admission["selected_profile"]
        if selected_profile != preflight["selected_profile_row"]["profile"]:
            raise ValueError(f"Stage211 {phase} correction batch-profile selection mismatch.")
    else:
        raise ValueError(f"Stage211 {phase} correction batch-profile binding is incomplete.")
    if any(
        (
...
def _validate_stage211_post_coverage_corrections(
    ...
    *,
    ...
    for round_index, correction in enumerate(corrections, start=1):
        ...
        batch_profile_preflight, batch_profile_admission = (
            _validate_stage211_correction_batch_profile(
                correction,
                phase=phase,
        ...
        selected_profile = batch_profile_preflight["selected_profile_row"]["profile"]
        replay_manifest = load_webdataset_bucket_manifest(replay_manifest_path)
        expected_steps_per_epoch = estimate_bucket_manifest_steps(
            replay_manifest,
            split="train",
            batch_size=int(selected_profile["batch_size"]),
            world_size=STAGE211_FULL_DATA_WORLD_SIZE,
        ...
        expected_tail_padding = estimate_bucket_manifest_tail_padding_samples(
            replay_manifest,
            split="train",
            batch_size=int(selected_profile["batch_size"]),
            world_size=STAGE211_FULL_DATA_WORLD_SIZE,
        ...
        if any(
            (
                ...
                tail_padding != expected_tail_padding,
                int(selected_coverage.get("steps_per_epoch", -1)) != expected_steps_per_epoch,
                int(selected_coverage.get("tail_padding_samples_per_epoch", -1))
                != expected_tail_padding,
            )
        ):
            raise ValueError(
                f"Stage211 {phase} correction round {round_index} profiled coverage mismatch."
            )
        _validate_bound_file(
            correction,
            path_key="completion_checkpoint_path",
        ...
        if round_index > STAGE211_RETENTION_CORRECTION_GUARANTEED_ROUNDS:
            extension_decision_path = _validate_bound_file(
                ...
                label=(f"Stage211 {phase} correction round {round_index} extension decision"),
            )
...
def _validate_stage211_segment_batch_profile(
    ...
    *,
    ...
    if schema_version != 2:
        raise ValueError(
            f"Stage211 {phase}/{difficulty} coverage schema is unsupported: {schema_version!r}"
        )
    from rwkvasr.eval.stage211_batch_profile import (
        validate_stage211_batch_profile_admission,
    )
...
def validate_stage211_full_data_coverage(
    ...
    *,
    ...
    for difficulty, expected in STAGE211_AUDIO_CURRICULUM.items():
        ...
        if int(segment.get("steps_per_epoch", -1)) != int(runtime_profile["steps_per_epoch"]):
        ...
        tail_padding_samples_per_epoch = int(runtime_profile["tail_padding_samples_per_epoch"])
    ...
    expected_combined_totals: dict[str, int | float] = {
        ...
        "total_tail_padding_sample_exposures": (
            original_tail_exposures + int(supplemental_profile["tail_padding_sample_exposures"])
        ),
        "total_executed_sample_exposures": (
            original_executed_exposures + int(supplemental_profile["executed_sample_exposures"])
        ),
    }
