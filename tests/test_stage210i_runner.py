...
stage210i = importlib.import_module("scripts.run_stage210i_balanced_hidden_ctc_distill")


def test_stage210i_changes_only_frame_balance_mode(tmp_path: Path) -> None:
    checkpoint = tmp_path / "stage210h.pt"
    checkpoint.touch()
    manifest = tmp_path / "manifest.json"
    manifest.write_text("{}", encoding="utf-8")
    segment = stage210i._segments(smoke=False)[0]
    config = stage210i._config(
        segment=segment,
    ...
    assert config["ctc_teacher_online_frame_balance_mode"] == "teacher_top1_balanced"
...
def test_stage210i_direct_entrypoint_is_runnable(tmp_path: Path) -> None:
    checkpoint = tmp_path / "stage210h.pt"
    checkpoint.touch()
    manifest = tmp_path / "manifest.json"
    manifest.write_text("{}", encoding="utf-8")
    result = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "run_stage210i_balanced_hidden_ctc_distill.py"),
            "--dry-run",
            "--smoke",
            "--init-checkpoint",
            str(checkpoint),
