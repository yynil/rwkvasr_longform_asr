...
stage210l = importlib.import_module("scripts.run_stage210l_stacked_hidden_realign")


def test_stage210l_isolates_stacked_hidden_path_alignment(tmp_path: Path) -> None:
    checkpoint = tmp_path / "selected.pt"
    checkpoint.touch()
    manifest = tmp_path / "manifest.json"
    manifest.write_text("{}", encoding="utf-8")
    segment = stage210l._segments(smoke=False)[0]
    config = stage210l._config(
        segment=segment,
...
def test_stage210l_direct_entrypoint_is_runnable(tmp_path: Path) -> None:
    checkpoint = tmp_path / "selected.pt"
    checkpoint.touch()
    manifest = tmp_path / "manifest.json"
    manifest.write_text("{}", encoding="utf-8")
    result = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "run_stage210l_stacked_hidden_realign.py"),
            "--dry-run",
            "--smoke",
            "--init-checkpoint",
            str(checkpoint),
    ...
    assert "phase=stacked_hidden" in result.stdout
