...
stage210m = importlib.import_module("scripts.run_stage210m_sequence_ctc_repair")


def test_stage210m_combines_sequence_factorized_and_stacked_repair(tmp_path: Path) -> None:
    checkpoint = tmp_path / "selected.pt"
    checkpoint.touch()
    manifest = tmp_path / "manifest.json"
    manifest.write_text("{}", encoding="utf-8")
    segment = stage210m._segments(smoke=False)[0]
    config = stage210m._config(
        segment=segment,
    ...
    assert config["lr"] == pytest.approx(5.0e-7)
    ...
    assert config["ctc_teacher_frame_filter"] == "nonblank_neighbors"
    ...
    assert config["ctc_teacher_online_sequence_loss_weight"] == pytest.approx(0.10)
    ...
    assert config["ctc_teacher_online_layer_sample_count"] == 12
    assert config["ctc_teacher_online_layer_include_boundaries"] is True
    assert config["ctc_teacher_online_layer_boundary_ids"] == list(stage210m.HARD_LAYER_IDS)
    ...
    assert config["ctc_teacher_online_layer_block_loss_weight"] == pytest.approx(0.5)
...
def test_stage210m_direct_entrypoint_is_runnable(tmp_path: Path) -> None:
    checkpoint = tmp_path / "selected.pt"
    checkpoint.touch()
    manifest = tmp_path / "manifest.json"
    manifest.write_text("{}", encoding="utf-8")
    result = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "run_stage210m_sequence_ctc_repair.py"),
            "--dry-run",
            "--smoke",
            "--init-checkpoint",
            str(checkpoint),
    ...
    assert "phase=sequence_ctc_repair" in result.stdout
